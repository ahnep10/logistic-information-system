...
updated: 2026-07-06T13:08:52Z
---

## Current Test

number: 1
name: Log in as a Manager and visit /dashboard
expected: |
  All 4 KPI tiles show plausible non-error numbers matching the DB (verified this session: Active Products=2, Active Suppliers=1, Movements Today=0, Low Stock=1); the PO Status panel shows a pie chart with a Received=4 slice (or the empty state if no POs exist); hovering a slice shows a tooltip; clicking a slice or the Low Stock tile navigates without a console error.
